...
2. **Select from completion menu**: The snippet appears in blink.cmp's completion menu
3. **Expand**: Press `<CR>` (Enter) to expand the snippet
4. **Navigate**: Use `<Tab>` and `<S-Tab>` to jump between snippet nodes
5. **Choice nodes**: Use `<C-l>` to cycle through alternatives in choice nodes
...
2. **Use fmt**: `fmt()` makes multiline snippets much easier to read
3. **Test incrementally**: Add one snippet at a time and test it
4. **Check filetype**: Use `:set filetype?` to verify the current filetype
5. **Use placeholders**: Good placeholder text helps remember what to type
6. **Document complex snippets**: Add comments explaining dynamic behavior
7. **Organize by category**: Group related snippets together in files
