...
local function parser_installed(parser)
	return (ts_available and treesitter_parsers.has_parser(parser)) or (vim.treesitter.query.get(parser, "highlights"))
end
...
if
	vim.islist(markview.configuration.buf_ignore)
	and vim.list_contains(markview.configuration.buf_ignore, vim.bo.buftype)
then
	return
end
...
local markview_augroup =
	vim.api.nvim_create_augroup("markview_buf_" .. vim.api.nvim_get_current_buf(), { clear = true })
...
vim.api.nvim_create_autocmd({ "ModeChanged", "TextChanged" }, {
	...
	callback = function(event)
		...
		if vim.islist(markview.configuration.modes) and vim.list_contains(markview.configuration.modes, mode) then
			...
			for _, window in ipairs(windows) do
				vim.wo[window].conceallevel = type(options.on_enable) == "table" and options.on_enable.conceallevel or 2
				vim.wo[window].concealcursor = type(options.on_enable) == "table" and options.on_enable.concealcursor
					or "n"
			...
			for _, window in ipairs(windows) do
				vim.wo[window].conceallevel = type(options.on_disable) == "table" and options.on_disable.conceallevel
					or markview.global_options.conceallevel
				vim.wo[window].concealcursor = type(options.on_disable) == "table" and options.on_disable.concealcursor
					or markview.global_options.concealcursor
			end
